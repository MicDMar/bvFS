          ...
          if(((cursors[y].block)*512 - cursors[y].pos) > check){
           ...
           // This means we are at the start of the block. 
            ...
            while(check > 0){
              ...
              // Set our cursor to its new position.
              cursors[y].pos = (new_block-1)*512;
              cursors[y].block = new_block;
              cursors[y].file_block++;
    ...
    if(strcmp(INode_array[i].file_name, current_open_file) == 0){
      ...
      for(int y = 0; y < sizeof(cursors)/sizeof(cursors[0]); y++){
        if(strcmp(cursors[y].file_name, current_open_file) == 0){
          ...
          //used to keep track of location to buffer we are reading and then
          //writing too
          int bufLoc = 0;
          if(check>maxBytes){
            ...
            while(check>BLOCK_SIZE && writtenBytes<maxBytes){
              ...
              //first time read
              if(flag==0){
                read(fsFD, buf+bufLoc, (((cursors[y].block+1)*512)-cursors[y].pos));
          ...
          //this should be fine? 
            ...
            if(flag == 0){
              ...
              if(check == 512){
                cursors[y].file_block++;
                cursors[y].pos = ((INode_array[i].file_blocks[cursors[y].file_block]-1)*512);
              } else {
                cursors[y].pos = ((INode_array[i].file_blocks[cursors[y].file_block]-1)*512 + check);
              }
              writtenBytes += check;
              bufLoc+=writtenBytes;
              return writtenBytes;
